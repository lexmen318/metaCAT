<template>
  <div class="paraphrasingBatchList">
    <ul class="batchItem-list">
      <el-row :gutter="15" justify="center" v-if="!noBatchData">
        <el-col :span="8" v-for="(batchItem, index) in allBatch" :key="index">
          <li class="batch-list-item">
            <div class="dialogue-id" @click="gotoAllparaphrasingDialogs(batchItem.batch_id)">{{batchItem.batch_id}} </div>
            ...
            <div class="batch-description">
              <template>
                ...
                <p class="add-description" @click="addDescription($event)" v-else>添加描述</p>
                <el-input
      ...
      <p v-else style="text-align: center;"> {{ $t('paraphrasingFunction.batch.lblNoDataHint') }}</p>
    </ul>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updataDescription (batchId, batchDesc){
      this.batch_description_post(batchId, batchDesc, "paraphrasing")
      .then(res => {
		...
		gotoAllparaphrasingDialogs(batchId){
			this.$router.push({
  ...
  created (){
    ...
    this.batch_list_by_category("paraphrasing")
      .then(res=>{
        this.allBatch = res.data
        this.allBatch.length > 0 ? (this.noBatchData = false) : (this.noBatchData = true)
      })
  }
}
</script>
